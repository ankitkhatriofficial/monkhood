	...
	<table>
	 ...
	 <c:forEach var="booking" items="${bookings}">
			<tr>
				...
				<td>${booking.propertyType eq '[]' ? 'ANY' : booking.propertyType}</td>
				<td>${booking.budget}</td>
				<td>${booking.furnishingReq}</td>
				<td>${booking.specialReq}</td>
				<td>${booking.refernce}</td>
				<td><c:if test="${booking.tenantReqStatus ne true}">
						<a
							href="${pageContext.request.contextPath}/u/withdraw/tenant-req/${booking.id}"
							onclick="return confirm('Are you sure to withdraw the Application?')">Withdraw</a>
					</c:if></td>
			</tr>
		</c:forEach>
	</table>
